...
async def test_authenticate_user(db: AsyncSession) -> None:
    ...
    user_in = UserCreate(email=email, password=password)
    user = await user_repository.create_user(session=db, user_create=user_in)
    authenticated_user = await user_repository.authenticate(session=db, email=email, password=password)
...
async def test_not_authenticate_user(db: AsyncSession) -> None:
    email = random_email()
    password = random_lower_string()
    user = await user_repository.authenticate(session=db, email=email, password=password)
    assert user is None
...
async def test_update_user(db: AsyncSession) -> None:
    ...
    if user.id is not None:
        await user_repository.update_user(session=db, db_user=user, user_in=user_in_update)
    user_2 = await db.get(User, user.id)
    assert user_2
    assert user.email == user_2.email
